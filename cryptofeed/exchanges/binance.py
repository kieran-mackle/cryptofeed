...
class Binance(Feed, BinanceRestMixin):
    ...
    async def _snapshot(self, pair: str) -> None:
        ...
        receipt_timestamp = (time.time_ns(), time.clock_gettime_ns(time.CLOCK_BOOTTIME))
        await self.book_callback(L2_BOOK, self._l2_book[std_pair], receipt_timestamp, timestamp=timestamp, raw=resp, sequence_number=self.last_update_id[std_pair])
